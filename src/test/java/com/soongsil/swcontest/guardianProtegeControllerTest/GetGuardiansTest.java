...
public class GetGuardiansTest extends BaseTest {
    ...
    @Test
    @DisplayName("환자를 등록한 보호자 조회 테스트(성공)")
    public void getGuardiansSuccess() throws Exception {
        ResultActions result = mockMvc.perform(get("/v1/getGuardian")
                .header("Authorization", signInResponseDto1.getAccessToken())
                .contentType(MediaType.APPLICATION_JSON));

        result.andExpect(status().isOk())
                .andExpect(jsonPath("guardiansCount").value(2));
    }

    @Test
    @DisplayName("환자를 등록한 보호자 조회 테스트(실패) - 없는 유저일 때")
    public void getGuardiansFailUserNotFound() throws Exception {
    ...
    @Test
    @DisplayName("환자를 등록한 보호자 조회 테스트(실패) - 보호자가 사용할 때")
    public void getGuardiansFailUserIsGuardian() throws Exception {
        ResultActions result = mockMvc.perform(get("/v1/getGuardian")
